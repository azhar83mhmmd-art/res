/*
 * PENTING: jangan require('@napi-rs/canvas') di top-level file ini.
 * File ini di-require oleh router/maker/bratvid.ts, yang di-import
 * statis oleh src/registry.ts sejak cold start. Kalau native binding
 * canvas gagal load, require() di top-level akan throw saat module
 * di-load dan MEMATIKAN SELURUH APLIKASI untuk semua endpoint (lihat
 * penjelasan lengkap di src/canvasSafe.ts). getCanvasLibLazy() di
 * bawah menunda require() sampai generateBratVideo() benar-benar
 * dipanggil.
 */
let _canvasLib = null;
function getCanvasLibLazy() {
  if (_canvasLib) return _canvasLib;
  _canvasLib = require('@napi-rs/canvas');
  return _canvasLib;
}

const { writeFileSync, existsSync, readFileSync, mkdtempSync, rmSync, mkdirSync } = require('fs')
...
const TEXT = "Brat vid 🎥"
const THEME = 'white'   // 'black' | 'white' | 'green'
const BLUR = 0          // 0, 1, 2, atau 3
const FORMAT = 'mp4'    // 'mp4' | 'gif'
const FRAME_DURATION = 0.4   // detik tiap kata/emoji muncul
const HOLD_DURATION = 1.2     // detik teks final tetap tampil di akhir
const MAX_WORD_PER_LAYER = 1  // jumlah kata/emoji yang muncul sekaligus tiap frame
const MAX_WORD_BEFORE_RESET = [7, 8]  // Maksimal kata sebelum layer reveal di-reset. Bisa angka tunggal (0 = tanpa reset) atau array custom per-batch, misal [5, 6] -> batch 1: 5 kata, batch 2: 6 kata, batch 3: balik ke 5 kata, dst
// Bisa true kalau mau render lebih cepat, tapi RAM lebih boros
const FAST_PROGRESS = true
...
/*
 * Bug sebelumnya: font & peta emoji di-cache di __dirname (folder source
 * code), yang read-only di lingkungan serverless seperti Vercel kecuali
 * /tmp — menyebabkan endpoint /api/maker/bratvid selalu gagal (EROFS)
 * begitu di-deploy. Dipindah ke os.tmpdir().
 */
const CACHE_DIR = path.join(os.tmpdir(), 'kairoo-bratvid')
const FONT_PATH = path.join(CACHE_DIR, 'ARIALN.ttf')
const EMOJI_JSON_PATH = path.join(CACHE_DIR, 'emoji-apple.json')
...
async function ensureFont() {
  ...
  getCanvasLibLazy().GlobalFonts.registerFromPath(FONT_PATH, 'ArialNarrow')
}
...
async function getEmojiImage(emoji) {
  ...
  if (!b64) return null
  const img = await getCanvasLibLazy().loadImage(Buffer.from(b64, 'base64'))
...
async function renderCanvas(text, theme, blurAmount) {
  ...
  const canvas = getCanvasLibLazy().createCanvas(size, size)
...
async function generateBratVideo({
  ...
  // Bug sebelumnya: ditulis ke process.cwd() (folder source code), yang
  // read-only di lingkungan serverless kecuali /tmp. Dipindah ke os.tmpdir().
  ...
  try {
    ...
    // ffmpeg mungkin tidak terpasang di environment ini (mis. serverless
    // tanpa binary ffmpeg) — lempar pesan yang jelas, bukan biarkan
    // stack trace mentah bocor ke response.
    throw new Error(
      `Gagal merender video (ffmpeg): ${err && err.message ? err.message : err}. ` +
      'Pastikan binary "ffmpeg" tersedia di environment/PATH server ini.'
    )
  }
